...
class DualURLProcessor:
    ...
    def add_url_columns(self):
        ...
        BEGIN TRY
            ...
            IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('CleanGrantsLayer2') AND name = 'URLProcessingNotes')
            BEGIN
                ALTER TABLE CleanGrantsLayer2 ADD URLProcessingNotes NVARCHAR(1000);
                PRINT 'Added URLProcessingNotes column';
            END
            ELSE
            BEGIN
                PRINT 'URLProcessingNotes column already exists';
            END
    ...
    def process_dual_urls(self):
        ...
        BEGIN TRY
            ...
            -- STEP 3: Add processing notes
            UPDATE CleanGrantsLayer2 
            SET 
                URLProcessingNotes = CONCAT(
                    'DirectApplyLink: ',
                    CASE 
                        WHEN OpportunityURL LIKE 'https://www.grants.gov/search-results-detail/%' THEN 'Perfect grants.gov direct link'
                        WHEN OpportunityURL LIKE 'https://%.gov/%' THEN 'Government HTTPS link'
                        WHEN OpportunityURL LIKE 'https://%' THEN 'Good HTTPS link'
                        WHEN OpportunityURL IS NULL OR OpportunityURL = '' THEN 'NULL - needs fallback'
                        ELSE 'Processed'
                    END,
                    ' | Url: ',
                    CASE 
                        WHEN AdditionalInfoURL LIKE 'https://%.gov/%' THEN 'Government info URL'
                        WHEN AdditionalInfoURL LIKE 'https://%' THEN 'Good info URL'
                        WHEN AdditionalInfoURL IS NULL OR AdditionalInfoURL = '' THEN 'NULL - optional'
                        ELSE 'Processed'
                    END
                ),
                URLProcessedDate = GETDATE();
            
            PRINT CONCAT('Added processing notes for ', @@ROWCOUNT, ' records');
            ...
            SET 
                ...
                URLProcessingNotes = REPLACE(URLProcessingNotes, 'DirectApplyLink: NULL - needs fallback', 'DirectApplyLink: Generated from OpportunityNumber')
            ...
            SET 
                ...
                UrlQuality = 65.0,
                URLProcessingNotes = REPLACE(URLProcessingNotes, 'Url: NULL - optional', 'Url: Generated from OpportunityURL')
    ...
    def create_dual_url_views(self):
        ...
        SELECT 
            ...
            UrlQuality,
            URLProcessingNotes,
            CASE WHEN RunweiCategory IS NOT NULL THEN RunweiCategory ELSE 'Uncategorized' END as RunweiCategory,
        ...
        SELECT 
            ...
            DirectApplyLinkQuality,
            UrlQuality,
            CASE WHEN RunweiCategory IS NOT NULL THEN RunweiCategory ELSE 'Grant' END as RunweiCategory,
            EstimatedTotalFunding,
            Deadline,
            DataQualityScore,
