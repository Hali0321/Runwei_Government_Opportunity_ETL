...
def main(mytimer: func.TimerRequest) -> None:
    logging.info('SearchGrants function triggered - UPDATED VERSION V6 (INCREASED DATA)')
    ...
    logging.info('Calling Search API: https://api.grants.gov/v1/api/search2')
    ...
    try:
        ...
        # Try different search methods to get better results - INCREASED ROWS FOR MORE DATA
        search_attempts = [
            {"method": "health search", "payload": {"keyword": "health", "rows": 100}},
            {"method": "education search", "payload": {"keyword": "education", "rows": 100}},
            {"method": "research search", "payload": {"keyword": "research", "rows": 100}},
            {"method": "science search", "payload": {"keyword": "science", "rows": 100}},
            {"method": "funding categories - health", "payload": {"fundingCategories": "HL", "rows": 100}},
            {"method": "funding categories - education", "payload": {"fundingCategories": "ED", "rows": 100}},
            {"method": "funding categories - science", "payload": {"fundingCategories": "ST", "rows": 100}},
            {"method": "recent grants", "payload": {"dateRange": "30", "rows": 100}},
            {"method": "agency search NIH", "payload": {"agencyCode": "HHS-NIH11", "rows": 100}},
            {"method": "agency search CDC", "payload": {"agencyCode": "HHS-CDC", "rows": 100}},
            {"method": "agency search NSF", "payload": {"agencyCode": "NSF", "rows": 100}},
            {"method": "blank search", "payload": {"rows": 200}}
        ]
        ...
        for attempt in search_attempts:
            ...
            # Make the API call
            response = requests.post("https://api.grants.gov/v1/api/search2", headers=headers, json=payload)
            
            # Log response info
            logging.info(f"API response status: {response.status_code}")
            if response.headers.get('Content-Type'):
                logging.info(f"API response content type: {response.headers.get('Content-Type')}")
            ...
            try:
                ...
                logging.info(f"API response keys: {list(search_results.keys())}")
